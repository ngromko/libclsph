  ...
  size_t sfaces;
	...
	for(size_t i = 0; i < shapes.size(); ++i) {
		...
		for(size_t j = 0; j < sfaces; ++j) {
            ...
            bbox.maxx=std::numeric_limits<unsigned int>::min();
            bbox.minx=std::numeric_limits<unsigned int>::max();
            bbox.maxy=std::numeric_limits<unsigned int>::min();
            bbox.miny=std::numeric_limits<unsigned int>::max();
            bbox.maxz=std::numeric_limits<unsigned int>::min();
            bbox.minz=std::numeric_limits<unsigned int>::max();
            ...
            bbox.size_x = std::ceil((bbox.maxx-bbox.minx)/distFieldThreshold * 2)+1;
            bbox.size_y = std::ceil((bbox.maxy-bbox.miny)/distFieldThreshold * 2)+1;
            bbox.size_z = std::ceil((bbox.maxz-bbox.minz)/distFieldThreshold * 2)+1;
